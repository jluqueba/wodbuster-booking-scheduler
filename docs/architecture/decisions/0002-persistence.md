...
- Journal mode: SQLite is opened with `journal_mode=DELETE` and `synchronous=NORMAL`. This is a change from the original decision to use `journal_mode=WAL`, discovered on 2026-07-02 during the first end-to-end deploy from GitHub Actions (see reasoning-log.md). WAL requires POSIX shared memory for its `.db-shm` file, which Azure Files SMB mounts do not reliably support; the concrete failure is `sqlite3.OperationalError: database is locked` on the first `CREATE TABLE` executed by Alembic. `DELETE` mode uses a plain `.db-journal` file (regular POSIX I/O) that SMB handles correctly. The trade-off is higher write latency, which is acceptable for the worker's write cadence (a handful of transactions per booking cycle) and further mitigated by the single-writer invariant.
