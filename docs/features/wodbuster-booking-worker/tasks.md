...
| F1.7 | [P] Author `docker-compose.yml` at the repo root providing one Postgres 16 service (image `postgres:16`, named volume for `pgdata`, exposed port 5432, local-only password from an `.env` file entry `POSTGRES_LOCAL_PASSWORD`). Update `.env.example` with `POSTGRES_HOST=localhost`, `POSTGRES_PORT=5432`, `POSTGRES_DB=wodbuster`, `POSTGRES_USER=wodbuster`, `POSTGRES_LOCAL_PASSWORD=<placeholder>`. Update `README.md` "Local development" section: `docker compose up -d postgres` before running the app or tests; `docker compose down -v` to reset. Add a note that no SQLite substrate exists in the codebase after the 2026-07-02 persistence pivot (see ADR-0002). This task supersedes the removed Azure Files local surrogate. | M | F1.1 |
...
| F3.3 | Author Bicep modules: `infra/modules/observability.bicep` (Log Analytics workspace plus Application Insights), `infra/modules/identity.bicep` (user-assigned managed identity plus role assignments for Key Vault Secrets User and AcrPull), `infra/modules/keyvault.bicep` (Key Vault with RBAC mode, soft-delete on), `infra/modules/registry.bicep` (Azure Container Registry Basic SKU). **Amended 2026-07-02 (persistence pivot)**: `infra/modules/storage.bicep` is removed from this module set (see F3.11 for its replacement, `postgres.bicep`). The storage account and Azure Files share are no longer part of the deployed footprint. | L | F3.2 | 0001, 0005, 0006 |
| F3.4 | Author `infra/modules/containerapp.bicep`: Container Apps environment, single Container App with `min-replicas=1`, `max-replicas=1`, single revision mode, UAMI bound, env vars wired to Key Vault references via `secretRef`. **Amended 2026-07-02 (persistence pivot)**: the `/data` volume mount (Azure Files) is removed. Add env vars for the Postgres connection: `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB`, `POSTGRES_USER` (the runtime UAMI's principal name) sourced from `postgres.bicep` outputs; no `POSTGRES_PASSWORD` env var (Entra token flow, see ADR-0005 "Postgres connection identity"). | M | F3.3, F3.11 | 0001, 0002, 0005 |
...
| F3.11 | Author `infra/modules/postgres.bicep`: Azure Database for PostgreSQL Flexible Server (`Microsoft.DBforPostgreSQL/flexibleServers@2024-08-01` or newer), SKU `Standard_B1ms` (Burstable), Postgres version 16, location `spaincentral`, availability zone `1`, storage 32 GiB with autogrow enabled, backup retention 7 days, geo-redundant backup disabled, high availability disabled. Public network access enabled. Firewall rules: an unconditional `AllowAllAzureServices` (0.0.0.0-0.0.0.0) rule for the runtime path (Container Apps Consumption env has no stable outbound IP — see ADR-0005 "Platform limitation" note) plus a parameter-driven operator IP CIDR for psql DBA access. `authConfig`: `activeDirectoryAuth = 'Enabled'`, `passwordAuth = 'Enabled'`, `tenantId` = current tenant. Admin login name `wodbadmin` with `@secure()` `administratorLoginPassword` parameter (value passed in by `main.bicep` from a Key Vault reference to `postgres-admin-password`, see F3.13). Declare an `administrators` sub-resource setting the operator's Entra user (parameter `entraDbaObjectId`, `entraDbaPrincipalName`) as the server's Entra admin. Emit outputs: `serverFqdn`, `databaseName`, `runtimeUamiPrincipalName` (the name the app will connect as). | L | F3.2, F3.3 | 0002, 0005 |
| F3.12 | **Operator manual setup, one-time after F3.11 lands.** Enrol the runtime UAMI as an Entra principal on the Postgres server: `az postgres flexible-server microsoft-entra-admin create --server-name pg-{token} -g {rg} --display-name {uamiName} --object-id {uamiPrincipalId} --type ServicePrincipal`. Note: the original plan (schema-scoped `GRANT USAGE, CREATE ON SCHEMA public` to a non-admin role via `pgaadauth_create_principal(...)`) is not achievable on this SKU today because the `pgaadauth` extension is not exposed to user control. The Entra admin binding gives the runtime UAMI `azure_pg_admin` (server admin) rights. See ADR-0005 "Platform limitation" note and reasoning-log 2026-07-02. Verify firewall rules by listing them with `az postgres flexible-server firewall-rule list`; the Bicep-managed `AllowAllAzureServices` rule handles the runtime path, and the operator's psql access needs an explicit rule with their current public IP (`az postgres flexible-server firewall-rule create --rule-name operator-home --start-ip-address <ip> --end-ip-address <ip>`). Without both this Entra enrolment and the AllowAllAzureServices rule the first container revision hangs at `alembic upgrade head`. | S | F3.11 | 0002, 0005 |
| F3.13 | **Operator manual setup, one-time before first `azd provision` with the Postgres module.** Populate the Key Vault secret `postgres-admin-password` with a 32-character random value (`openssl rand -base64 32`). This secret is consumed only by `main.bicep` at provision time to seed the Postgres server's `administratorLoginPassword`. Never mounted into the Container App. Break-glass only. | S | F3.8 | 0002, 0005 |
| F3.14 | **Cost re-check, one-time after the first billing cycle following the F3.11 deploy.** Pull the resource group's cost breakdown via `az consumption usage list` or the Cost Management blade. Compare the actual monthly Postgres line item against the 13-15 EUR ballpark in ADR-0002. If the delta exceeds 25 percent, open a follow-up work item to either downgrade the SKU or move the workload to a different pricing model. Not a recurring task; single audit after the pivot. | S | F3.11 | 0002 |

### F4: Persistence and security primitives (Phase 3)

| ID | Task | Size | Dependencies | ADRs / FRs |
|----|------|------|--------------|------------|
| F4.1 | Implement SQLAlchemy models in `src/wodbuster_worker/persistence/models.py` for all ten tables from [plan.md](plan.md) (`operator_profile`, `federated_identity`, `scheduler_rule`, `class_preference`, `cookie_credential`, `booking_outcome`, `vacation_window`, `heartbeat_reading`, `alert`, `notification_outbox`). Use Postgres-native types where the shape calls for them: `JSONB` for `notification_outbox.payload`, `alert.payload`, and `booking_outcome.response_payload`; native `ENUM` (`sa.Enum(..., native_enum=True)`) for enumerated status columns; `TIMESTAMPTZ` for all timestamps. Enforce unique constraint `(operator_id, kind)` on open alerts via `postgresql_where=text("closed_at IS NULL")` (replacing the previous SQLite-only `sqlite_where=`). No SQLite dialect hints anywhere in the module. | M | F1.2 | 0002 |
| F4.2 | Configure the Postgres engine in `src/wodbuster_worker/persistence/engine.py` using `psycopg[binary]` version 3, sync API. Compose the URL from `Settings.postgres_host`, `postgres_port`, `postgres_db`, `postgres_user`. Register a `do_connect` event listener that resolves the runtime UAMI's Entra access token via `DefaultAzureCredential.get_token("https://ossrdbms-aad.database.windows.net/.default")` and injects it as the connection password (in local dev with `WODBUSTER_ENV=local`, the listener falls back to a static password from `POSTGRES_LOCAL_PASSWORD`). Keep the `sessionmaker` configuration unchanged. Delete the `_install_pragmas` SQLite listener; delete `check_same_thread` and any other SQLite-specific `connect_args`. | M | F4.1, F1.7 | 0002, 0005 |
| F4.3 | Delete the existing SQLite baseline migration at `alembic/versions/c7808dfff47d_baseline.py`. Regenerate a fresh baseline against a live Postgres 16 instance (started via `docker compose up -d postgres`) using `alembic revision --autogenerate -m 'baseline'`. Hand-groom the generated file so DDL uses Postgres idioms (`JSONB`, native `ENUM` `CREATE TYPE`, `postgresql_where=` on the partial unique index). Update `alembic/env.py` to remove `render_as_batch=True` (SQLite-only) and to build the URL from the Postgres settings. Wire `alembic upgrade head` into the container startup script before `uvicorn` launches, running under the runtime UAMI's Entra identity (relies on F3.12 grants). | M | F4.1, F4.2, F1.7 | 0002 |
...
| F4.T2 | Component test that Alembic `upgrade head` against a fresh Postgres 16 database (spun up via `docker compose up -d postgres` at session scope, or via `testcontainers-python` for per-session isolation) produces every table from F4.1 and that a basic insert plus select round-trips on each. The test tears down the database (or drops the schema) between runs to keep isolation. No SQLite fallback path. | M | F4.3, F1.7 |
...
**Foundational complexity rollup**: 7 S + 5 M + 2 L plus 3 S/M tests. Total: roughly 2 to 2.5 weeks of solo dev time including the operator-driven manual setup (OAuth, Telegram, Healthchecks.io, Key Vault seeding, Postgres Entra admin + role grant) and the first `azd up` round-trip. The persistence pivot (2026-07-02) adds five tasks: F1.7 (docker-compose), F3.11 (postgres.bicep), F3.12 (Entra admin + role grant), F3.13 (`postgres-admin-password` KV seed), F3.14 (cost re-check).
...
| Bucket | Tasks | Tests |
|--------|-------|-------|
| Foundational (F1 through F4) | 29 | 3 |
| US-001 | 10 | 6 |
| US-002 | 7 | 5 |
| US-003 | 7 | 5 |
| US-004 | 4 | 5 |
