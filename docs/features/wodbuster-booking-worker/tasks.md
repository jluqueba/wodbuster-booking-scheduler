...
| F3.12 | **Operator manual setup, one-time after F3.11 lands.** Using the operator's Entra credentials, run `az postgres flexible-server ad-admin create ...` to confirm the Entra admin binding (idempotent with F3.11 declaration). Then connect via `psql` using an Entra access token (`az account get-access-token --resource-type oss-rdbms`) and execute, against the target database: `CREATE ROLE "<runtime-uami-principal-name>" WITH LOGIN IN ROLE azure_ad_user;` if the role does not already exist, followed by `GRANT CONNECT ON DATABASE wodbuster TO "<runtime-uami-principal-name>";` and `GRANT USAGE, CREATE ON SCHEMA public TO "<runtime-uami-principal-name>";`. This is the one-time seed that lets Alembic run under the runtime UAMI's identity at container startup. Document the exact commands in the README. Without this task the first container revision fails to boot. | M | F3.11 | 0002, 0005 |
