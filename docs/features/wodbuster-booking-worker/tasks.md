...
| F2.2 | Author `.github/workflows/deploy.yml` triggered on push to `main` after CI green. Builds and pushes the container image to ACR, then runs `azd deploy`. Uses OIDC federation against the user-assigned managed identity (no client secrets in GitHub per ADR-0005). | M | F1.5, F3.5 |
...
| F3.5 | Run `azd init`, `azd env new prod`, `azd up` against the operator subscription. Verify the Container App responds on `/health`. Capture the FQDN. | M | F3.4 | 0007 |
